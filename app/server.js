...
// Flag planting at boot time was removed as requested.
// Flags will be planted dynamically when PWNED via getFlag().

// Challenge Isolation Middleware
if (process.env.CHALLENGE_MODE) {
  const allowedPath = process.env.CHALLENGE_MODE.toLowerCase();
  console.log(`[ISOLATION] Restricting app to only serve: ${allowedPath}`);
  
  app.use((req, res, next) => {
    // Always allow static files, images, and basic views (like index)
    if (req.path.match(/\.(js|css|png|jpg|jpeg|gif|ico|svg|ttf|woff|woff2|ejs)$/i) || req.path === '/' || req.path.startsWith('/image')) {
      return next();
    }
    
    // Allow the specific challenge path
    if (req.path.toLowerCase().includes(allowedPath)) {
      return next();
    }
    
    // Everything else block
    res.status(404).send(`[Container Isolated] Only ${allowedPath} is active in this challenge system.`);
  });
}
...
if (SOCKET_PATH) {
  ...
  app.listen(SOCKET_PATH, () => {
    ...
    pushLine('info', `vuln-web startup on unix socket ${SOCKET_PATH}`, {
      socketPath: SOCKET_PATH,
      maxLines: MAX_LOG_LINES
    });
    console.log(`⚠️  VULNERABLE APP running on unix socket ${SOCKET_PATH}`);
