...
router.get('/oauth/silver', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <title>OAuth Connect</title>
      <style>
    ...
    <body>
      <div class="card">
        <h1>Connect With Social Login</h1>
        <p>The callback handler accepts an authorization code without validating <code>state</code>.</p>
        <form action="/oauth/silver" method="post">
          <input type="hidden" name="code" value="auth_victim_code" />
          <button type="submit">Continue As Victim</button>
        </form>
      </div>
      <div class="card">
        <h2>Attack Flow</h2>
        <pre>1. Victim authorizes the app and gets code=auth_victim_code
2. Attacker replays the callback to /oauth/silver
3. Missing state validation returns the victim token</pre>
      </div>
    </body>
    </html>
  `);
